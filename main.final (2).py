...
def MainSelectionFunction():
    city = combo1.get()
    selected_type = combo2.get()
    graphic_type = combo3.get()
    label_templates = {
        "Однокімнатна, Оренда": "{} Однокімнатна, Оренда,грн",
        "Двохкімнатна,Оренда": "{} Двохкімнатна, Оренда,грн",
        "Ціна за метр квадратний": "{} Ціна за метр квадратний,грн"
    }
    if city == "Київ":
        label2 = "Київ Ціна за метр квадратний,грн"
    elif city == "Львів":
        label2 = "Львів Ціна за метр квадратний,грн"
    elif city == "Одеса":
        label2 = "Одеса Ціна за метр квадратний,грн"

    if selected_type == "Усі":
        for template in label_templates.values():
            label = template.format(city)
            if graphic_type == "Звичайний графік":
                plot_histogram(label)
            elif graphic_type == "Графік змін цін":
                plot_price_differences(label)
            elif graphic_type == "Екстраполяції та Апроксимації":
                exstra_aprox(label)
            elif graphic_type == "Лінійна Регресія":
                linear_regression(label, label2)
    elif selected_type in label_templates:
        label = label_templates[selected_type].format(city)
        if graphic_type == "Звичайний графік":
            plot_histogram(label)
        elif graphic_type == "Графік змін цін":
            plot_price_differences(label)
        ...
        matching_column = FindColum(city, selected_type)
...
combo3 = ttk.Combobox(window, values=["Звичайний графік", "Графік змін цін", "Екстраполяції та Апроксимації", "Лінійна Регресія"], width=20,font=("Helvetica", 14))
